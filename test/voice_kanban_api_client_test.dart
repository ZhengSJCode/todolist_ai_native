...
void main() {
  group('VoiceKanbanApiClient', () {
    ...
    test('C6: createEntry sends sourceType voice when requested', () async {
      ...
      final res = await client.createEntry('test text', sourceType: 'voice');
      expect(res.rawEntry.id, 'entry-voice-1');
      ...
      final body = jsonDecode(captured.last as String) as Map<String, dynamic>;
      expect(body['sourceType'], 'voice');
    });

    test('C3: listItems without type filter requests type=all', () async {
      when(
        mockHttpClient.get(Uri.parse('$baseUrl/items?type=all')),
    ...
    test(
      'C4: listItems with metric type filter requests type=metric',
      () async {
        when(
          mockHttpClient.get(Uri.parse('$baseUrl/items?type=metric')),
        ).thenAnswer((_) async => http.Response('[]', 200));
    ...
    test(
      'C5: 400 response throws ApiException with server error message',
      () async {
        when(
          mockHttpClient.post(
            any,
          ...
          (_) async =>
              http.Response(jsonEncode({'error': 'Server error details'}), 400),
        );
